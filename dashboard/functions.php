    ...
    if (isset($_POST['Operation'])) {
        if ($_POST['Operation'] ==  'filterNotResultPosted') {
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                foreach ($filteredNotResultPostedArray as $filteredNotResultPostedRows) {
                ?>
                    <tr>
                        <td>
                            <a class='classAnchorTag' href="stucountview.php?classId=<?php echo $filteredNotResultPostedRows['cls_id'] ?>">
                                <?php echo $filteredNotResultPostedRows['cls_course'] . ' ' . $filteredNotResultPostedRows['cls_dept'] . ' ' . $filteredNotResultPostedRows['cls_sem'] . ' (' . $filteredNotResultPostedRows['cls_startyr'] . ' - ' . $filteredNotResultPostedRows['cls_endyr'] . ')' ?>
                            </a>
                        </td>
                        <td><?php echo $filteredNotResultPostedRows['sub_name'] ?></td>
                        <td><?php echo $filteredNotResultPostedRows['ce_exam'] ?></td>
                        <td><?php echo $filteredNotResultPostedRows['f_fname'] . ' ' . $filteredNotResultPostedRows['f_lname'] ?></td>
                    </tr>
                <?php
                }
            } else {
